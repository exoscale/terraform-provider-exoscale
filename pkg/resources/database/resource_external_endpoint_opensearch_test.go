...
type TemplateModelExternalEndpointOpensearch struct {
	ResourceName string
	Name         string
	Zone         string
	URL          string
...
func testResourceExternalEndpointOpensearch(t *testing.T) {
	t.Parallel()

	tpl, err := template.ParseFiles("testdata/resource_external_endpoint_opensearch.tmpl")
	if err != nil {
		t.Fatal(err)
	}
	...
	base := TemplateModelExternalEndpointOpensearch{
		ResourceName: "test",
		Name:         rawName,
		Zone:         testutils.TestZoneName,
		URL:          "https://test-opensearch.example.com:9200",
