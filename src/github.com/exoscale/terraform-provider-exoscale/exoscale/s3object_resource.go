...
func s3ObjectRead(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := bucket.List(d.Get("key").(string), "/", "", 100)
	if err != nil {
		return err
	}
...
func s3ObjectLoadData(d *schema.ResourceData, bucket *s3.Bucket) error {
	if d.Get("content") != "" {
		...
		err := bucket.Put(d.Get("key").(string), buffer.Bytes(),
			d.Get("type").(string), ConvertAcl(d.Get("acl").(string)))
		if err != nil {
			return err
		}
	} else {
		file, err := os.Open(d.Get("source").(string))
		if err != nil {
			return err
		}
		...
		rf, err := ioutil.ReadFile(d.Get("source").(string))
		if err != nil {
			return err
		}
